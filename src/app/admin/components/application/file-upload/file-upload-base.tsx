...
import { Button } from "@/admin/components/base/buttons/button";
import { ButtonUtility } from "@/admin/components/base/buttons/button-utility";
import { ProgressBar } from "@/admin/components/base/progress-indicators/progress-indicators";
import { FeaturedIcon } from "@/admin/components/foundations/featured-icon/featured-icon";
...
export const FileUploadDropZone = ({
    ...
    return (
        <div
            ...
            onDrop={handleDrop}
            className={cx(
                "relative flex flex-col items-center gap-3 rounded-xl bg-gray-1 dark:bg-dark-3/50 border border-dashed border-gray-300 dark:border-dark-5 px-6 py-4 text-gray-500 dark:text-gray-400 transition duration-100 ease-linear",
                isDraggingOver && "border-solid border-primary bg-primary/5",
        ...
        >
            <FeaturedIcon 
                ...
                theme="light" 
                size="md" 
                className={cx("bg-white! shadow-sm ring-1 ring-gray-100", isDisabled && "opacity-50")} 
            />
            ...
            <div className="flex flex-col gap-1 text-center">
                ...
                <p className={cx("text-xs transition duration-100 ease-linear", isInvalid && "text-red-500")}>
...
export const FileListItemProgressBar = ({ name, size, progress, failed, type, fileIconVariant, onDelete, onRetry, className }: FileListItemProps) => {
    ...
    return (
        <motion.li
            layout="position"
            className={cx(
                "relative flex gap-3 rounded-xl bg-white dark:bg-dark-3 p-4 border border-gray-200 dark:border-dark-5 transition-shadow duration-100 ease-linear",
                ...
                <div className="flex w-full max-w-full min-w-0 flex-1">
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-0.5 flex items-center gap-2">
                            <p className="truncate text-sm whitespace-nowrap text-gray-500 dark:text-gray-400">{getReadableFileSize(size)}</p>

                            <hr className="h-3 w-px rounded-t-full rounded-b-full border-none bg-gray-200 dark:border-gray-700" />

                            <div className="flex items-center gap-1">
                                {isComplete && <CheckCircle className="size-4 stroke-[2.5px] text-emerald-500" />}
                                {isComplete && <p className="text-sm font-medium text-emerald-500">Complete</p>}

                                {!isComplete && !failed && <UploadCloud02 className="size-4 stroke-[2.5px] text-gray-400" />}
                                {!isComplete && !failed && <p className="text-sm font-medium text-gray-500">Uploading...</p>}

                                {failed && <XCircle className="size-4 text-red-500" />}
                                {failed && <p className="text-sm font-medium text-red-500">Failed</p>}
                            </div>
                        </div>
                    </div>
...
export const FileListItemProgressFill = ({ name, size, progress, failed, type, fileIconVariant, onDelete, onRetry, className }: FileListItemProps) => {
    ...
    return (
        <motion.li layout="position" className={cx("relative flex gap-3 overflow-hidden rounded-xl bg-white dark:bg-dark-3 p-4 border border-gray-200 dark:border-dark-5 transition-shadow duration-100 ease-linear", failed && "border-red-500", className)}>
            {/* Progress fill. */}
            <div
                style={{ transform: `translateX(-${100 - progress}%)` }}
                className={cx("absolute inset-0 size-full bg-gray-50 dark:bg-gray-900 transition duration-75 ease-linear", isComplete && "opacity-0")}
                role="progressbar"
            ...
            <div className="relative flex min-w-0 flex-1">
                <div className="relative flex min-w-0 flex-1 flex-col items-start">
                    <div className="w-full min-w-0 flex-1">
                        ...
                        <div className="mt-0.5 flex items-center gap-2">
                            <p className="text-sm text-gray-500 dark:text-gray-400">{failed ? "Upload failed, please try again" : getReadableFileSize(size)}</p>

                            {!failed && (
                                <>
                                    <hr className="h-3 w-px rounded-t-full rounded-b-full border-none bg-gray-200 dark:bg-gray-700" />
                                    <div className="flex items-center gap-1">
                                        {isComplete && <CheckCircle className="size-4 stroke-[2.5px] text-emerald-500" />}
                                        {!isComplete && <UploadCloud02 className="size-4 stroke-[2.5px] text-gray-400" />}
