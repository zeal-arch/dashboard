        ...
        <AriaTooltipTrigger {...{ trigger, delay, closeDelay, isDisabled, isOpen, defaultOpen, onOpenChange }}>
            ...
            >
                {({ isEntering, isExiting }) => (
                    <div
                        className={cx(
                            "z-1000 flex max-w-xs origin-(--trigger-anchor-point) flex-col items-start gap-1 rounded-lg px-3 shadow-lg will-change-transform",
                            "bg-gray-500/80 dark:bg-gray-200", // Background colors
                    ...
                    >
                        <span className="text-xs font-semibold text-white dark:text-gray-900">{title}</span>

                        {description && <span className="text-xs font-medium text-gray-200 dark:text-gray-600">{description}</span>}

                        {arrow && (
                            <AriaOverlayArrow>
                                <svg
                                    viewBox="0 0 100 100"
                                    className="size-2.5 fill-gray-500 dark:fill-gray-200 in-placement-left:-rotate-90 in-placement-right:rotate-90 in-placement-top:rotate-0 in-placement-bottom:rotate-180"
                                >
